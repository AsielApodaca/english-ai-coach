...
test("completeWithFallback: falls back to the next provider on failure", async () => {
  const result = await completeWithFallback([failing("zen"), fake("gemini", "ok"), fake("ollama", "ignored")], [{ role: "user", content: "hi" }]);
  assert.equal(result.provider, "gemini");
  assert.equal(result.text, "ok");
});

test("completeWithFallback: uses the first working provider", async () => {
  const result = await completeWithFallback([fake("zen", "from zen"), fake("gemini", "from gemini")], [{ role: "user", content: "hi" }]);
  assert.equal(result.provider, "zen");
});

test("completeWithFallback: throws combined error when all fail", async () => {
  await assert.rejects(
    () => completeWithFallback([failing("zen"), failing("gemini")], [{ role: "user", content: "hi" }]),
    /All LLM providers failed/,
  );
});

test("chatJSON: extracts structured data from response with prose", async () => {
  const { data, provider } = await chatJSON<{ word: string }>([fake("zen", 'Here you go: ```json\n{"word": "situation"}\n```')], {
    system: "be strict",
    user: "task",
  });
...
test("generatePracticeSet: builds fragments from LLM JSON", async () => {
  const { set, provider } = await generatePracticeSet([fake("zen", FAKE_SET)], {
    category: "star",
    level: "B2",
    learnerMemory: "",
  });
...
test("evaluateFragment: blends deterministic match with LLM naturalness", async () => {
  ...
  const { evaluation, provider } = await evaluateFragment([fake("zen", FAKE_EVAL)], {
    target,
    userText: user,
    question: "q",
    level: "B2",
...
test("evaluateFragment: still returns deterministic feedback when LLM fails", async () => {
  ...
  const { evaluation } = await evaluateFragment([failing("zen"), failing("gemini")], {
    target,
    userText: user,
    question: "q",
    level: "B2",
