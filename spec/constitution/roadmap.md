...
3. **003 · Conversación hablada natural** — el chat libre pasa a conversación viva hablada (mic → STT whisper/browser → `/api/chat` con contexto → TTS neural); correcciones sobre la marcha; modo Interview con 4-6 preguntas encadenadas, follow-ups y puntuación compuesta; la conversación se persiste como sesión (`turns`) y alimenta memoria/next-step. Usa 002 (STT whisper) y 007 (voz neural).
4. **007 · Voz humana (TTS neural local)** — voz neural natural vía Piper local (`piper` subprocess, stdin), fallback a edge-tts online y `speechSynthesis` como último recurso; pausas medidas entre fragmentos y velocidad por `length_scale`; estado de TTS visible en Settings.
